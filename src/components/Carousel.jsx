    ...
    <Slider {...settings}>
      {
        data?.slice(0,7)?.map((item, index) => {
          return <div key={index} className='bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] -z-10'>
            <div className="flex flex-col md:flex-row gap-10 justify-center h-[450px] my-20 md:my-0 items-center px-4">
              ...
              <button className="bg-gradient-to-r from-red-500 to-purple-500 text-white px-3 py-2 rounded-md cursor-pointer mt-2">Shop Now</button>
               ...
               <div>
              <img src={item.image} alt={item.title} className="rounded-full w-[400px] hover:scale-105 transition-all shadow-2xl shadow-red-400" />
            </div>
            </div>
